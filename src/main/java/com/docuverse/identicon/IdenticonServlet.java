...
/**
 * This servlet generates <i>identicon</i> (visual identifier) images ranging from 16x16 to 64x64 in size.
 * 
 * <h5>Supported Image Formats</h5>
 * <p>
 * Currently only PNG is supported because <code>javax.imageio</code> package does not come with built-in GIF encoder and PNG is the only remaining reasonable format.
 * </p>
 * <h5>Initialization Parameters:</h5> <blockquote>
 * <dl>
 * <dt>inetSalt</dt>
 ...
 * </dl>
 * </blockquote> <h5>Request ParametersP</h5> <blockquote>
 * <dl>
 * <dt>code</dt>
 * <dd>identicon code to render. If missing, requester's IP addresses is used to generated one. (Optional)</dd>
 * <dt>size</dt>
 * <dd>identicon size in pixels. If missing, a 16x16 pixels identicon is returned. Minimum size is 16 and maximum is 64. (Optional)</dd>
 * </dl>
 * </blockquote>
 * 
 * @author don
...
public class IdenticonServlet extends HttpServlet {
	...
	private long identiconExpiresInMillis = IdenticonServlet.DEFAULT_IDENTICON_EXPIRES_IN_MILLIS;
	...
	public void init(ServletConfig cfg) throws ServletException {
		...
		// visual changes.
		if (cfg.getInitParameter(IdenticonServlet.INIT_PARAM_VERSION) != null) {
			this.version = Integer.parseInt(cfg.getInitParameter(IdenticonServlet.INIT_PARAM_VERSION));
		}

		String inetSalt = cfg.getInitParameter(IdenticonServlet.INIT_PARAM_INET_SALT);
		if (inetSalt != null && inetSalt.length() > 0) {
			IdenticonUtil.setInetSalt(inetSalt);
		} else {
			throw new ServletException(IdenticonServlet.INIT_PARAM_INET_SALT + " init parameter not set");
		}

		String cacheProvider = cfg.getInitParameter(IdenticonServlet.INIT_PARAM_CACHE_PROVIDER);
		if (cacheProvider != null) {
			try {
				Class cacheClass = Class.forName(cacheProvider);
				this.cache = (IdenticonCache) cacheClass.newInstance();
			} catch (Exception e) {
				IdenticonServlet.log.error(e);
			}
		}
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String codeParam = request.getParameter(IdenticonServlet.PARAM_IDENTICON_CODE_SHORT);
		if (codeParam == null) {
			codeParam = request.getParameter(IdenticonServlet.PARAM_IDENTICON_CODE);
		}
		...
		String sizeParam = request.getParameter(IdenticonServlet.PARAM_IDENTICON_SIZE_SHORT);
		if (sizeParam == null) {
			sizeParam = request.getParameter(IdenticonServlet.PARAM_IDENTICON_SIZE);
		}
		...
		String identiconETag = IdenticonUtil.getIdenticonETag(code, size, this.version);
		...
		if (requestETag != null && requestETag.equals(identiconETag)) {
			...
			if (this.cache == null || (imageBytes = this.cache.get(identiconETag)) == null) {
				ByteArrayOutputStream byteOut = new ByteArrayOutputStream();
				RenderedImage image = this.renderer.render(code, size);
				ImageIO.write(image, IdenticonServlet.IDENTICON_IMAGE_FORMAT, byteOut);
				imageBytes = byteOut.toByteArray();
				if (this.cache != null) {
					this.cache.add(identiconETag, imageBytes);
				}
			}
			...
			if (codeSpecified) {
				long expires = System.currentTimeMillis() + this.identiconExpiresInMillis;
				response.addDateHeader("Expires", expires);
			}

			// return image bytes to requester
			response.setContentType(IdenticonServlet.IDENTICON_IMAGE_MIMETYPE);
